...
function dbConnect(){
  ...
  $dsn = 'mysql:dbname=ramune;host=localhost;charset=utf8';
  $user = 'root';
  $password = '';
  $options = array(
...
// function getUser2($u_id){
//   try{
//     $dbh = dbConnect();
//     $sql = 'SELECT user.username FROM message JOIN user ON message.from_user = users.id';
//     $data = array(':u_id' => $u_id);
//     //クエリ実行
//     $stmt = queryPost($dbh,$sql,$data);
//     //クエリ結果の１データを返却
...
//   try{
//     $dbh = dbConnect();
//     $sql = 'SELECT username FROM message JOIN users on massage.from_user = users.id';
//     $data = array();
//     $stmt = queryPost($dbh,$sql,$data);
//   }catch (Exception $e){
//     error_log('エラー発生：' . $e->getMessage());
